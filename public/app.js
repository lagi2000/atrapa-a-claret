...
(() => {
 ...
 function savePersist(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(persist));}catch{/* Play continues without persistent storage. */}}
  ...
  function showScreen(id){if(id!=='screen-game'){clearTimeout(answerTimer);questionToken++;}document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));document.getElementById(id).classList.add('active');const home=document.getElementById('homeToggle');if(home)home.hidden=id==='screen-home';const sound=document.getElementById('soundToggle');if(sound)sound.hidden=id==='screen-home';document.body.dataset.screen=id;window.scrollTo(0,0);const el=document.getElementById(id);el.tabIndex=-1;el.focus({preventScroll:true});}
